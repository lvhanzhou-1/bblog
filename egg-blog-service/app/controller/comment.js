...
class CommentController extends Controller {
    ...
    async queryCommentCount() {
        ...
        let result = await ctx.service.comment.queryCommentAll()
        let res
        if (result.length) {
            res = result.length
        } else {
    ...
    async saveComment() {
        ...
        if (!isValid) { // 校验不通过
             ...
             *
             */
            let flag = await ctx.service.comment.saveComment(params)
            if (flag) {
                ctx.success()
            }
        }
    ...
    async deleteCommentByUid() {
        ...
        if (!isValid) { // 校验不通过
            ...
            let result = await ctx.service.comment.deleteCommentByUid(uids)
            if (result) {
                ctx.success()
            }
        }
     ...
     * （除非中间表，不需要维护主键，选择id自增，由数据库自己维护）
     */
    async passOrRejectCommentByUid() {
        const {ctx} = this
        let uids = ctx.request.body
        ...
        if (!isValid) { // 校验不通过
            ...
            let result = await ctx.service.comment.passOrRejectCommentByUid(uids, commentStatus)
            if (result) {
                ctx.success()
            }
        }
    ...
    async queryCommentPage() {
        ...
        if (!isValid) { // 校验不通过
            ...
            let result = await ctx.service.comment.queryCommentPage(params)
            let {total} = await ctx.service.comment.queryAllCountComment(params)
            let newResult = {
                result,
                total,
                currentPage,
    ...
    async queryComment() {
        ...
        * */
        ...
        // 尚未处理的留言
        let result = await ctx.service.comment.queryComment(params)
        // 将违规评论特殊处理
        let result2 = ctx.service.comment.convertIllegalComment(result)

        let finalResult

        if (!userId) { // 如果没有userId，说明是游客
            // 返回游客可以看到的评论列表
            finalResult = ctx.service.comment.commentAboutVisitorsCanSee(result2)
        } else { // 反之，说明是已登录的用户

            // 返回用户可以看到的评论列表
            finalResult = ctx.service.comment.commentAboutUserCanSee(result2, userId)
        }
    ...
    async updateCommentByUid() {
        ...
        if (!isValid) { // 校验不通过
            ...
            // 补充参数
            let {create_time} = await ctx.service.comment.queryCommentByUid(uid)
            params.createTime = create_time

            // 根据uid修改这条记录
            let result = await ctx.service.comment.updateCommentByUid(params)
            if (result) {
                ctx.success()
            }
        }
