...
class SpecialPartSectionBlogController extends Controller {

    async saveSpecialPartSectionBlog() {
        const {ctx} = this
        ...
        if (!isValid) { // 校验不通过
             ...
             *
             */
            let result = await ctx.service.specialPartSectionBlog.querySpecialPartSectionBlogByPartSectionId(partSectionId, blogId)
            ...
            if (result) { // 已存在，不保存
                ...
                let flag = await ctx.service.specialPartSectionBlog.saveSpecialPartSectionBlog(params)
                if (flag) {
                    ctx.success()
                }
            }
     ...
     */
    async deleteSpecialPartSectionBlogByUid() {
        const {ctx} = this
        ...
        if (!isValid) { // 校验不通过
            ...
            let result = await ctx.service.specialPartSectionBlog.deleteSpecialPartSectionBlogByUid(uids)
            if (result) {
                ctx.success()
            }
        }
    ...
    async querySpecialPartSectionBlogPage() {
        const {ctx} = this
        ...
        if (!isValid) { // 校验不通过
            ...
            let result = await ctx.service.specialPartSectionBlog.querySpecialPartSectionBlogPage(params)
            let {total} = await ctx.service.specialPartSectionBlog.queryAllCountSpecialPartSectionBlog(params)
            let newResult = {
                result,
                total,
                currentPage,
    ...
    async updateSpecialPartSectionBlogByUid() {
        const {ctx} = this
        ...
        if (!isValid) { // 校验不通过
            ...
            // 补充参数
            let {create_time} = await ctx.service.specialPartSectionBlog.querySpecialPartSectionBlogByUid(uid)
            params.createTime = create_time

            // 根据uid修改这条记录
            let result = await ctx.service.specialPartSectionBlog.updateSpecialPartSectionBlogByUid(params)
            if (result) {
                ctx.success()
            }
        }
    ...
    async querySpecialPartSectionBlogAll() {
        const {ctx} = this
        console.log(ctx.request.body, 'ctx.request.body')
        let result = await ctx.service.specialPartSectionBlog.querySpecialPartSectionBlogAll()
        ctx.success(result)
    }
}

module.exports = SpecialPartSectionBlogController;
