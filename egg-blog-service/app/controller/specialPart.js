...
class SpecialPartController extends Controller {
    ...
    async saveSpecialPart() {
        ...
        if (!isValid) { // 校验不通过
             ...
             *
             */
            let result = await ctx.service.specialPart.querySpecialPartByPartName(partName, specialId)
            ...
            if (result) { // 已存在，不保存
                ...
                let flag = await ctx.service.specialPart.saveSpecialPart(params)
                if (flag) {
                    ctx.success()
    ...
    async deleteSpecialPartByUid() {
        ...
        if (!isValid) { // 校验不通过
            ...
            let result = await ctx.service.specialPart.deleteSpecialPartByUid(uids)
            if (result) {
                ctx.success()
            }
        }
    ...
    async querySpecialPartPage() {
        ...
        if (!isValid) { // 校验不通过
            ...
            let result = await ctx.service.specialPart.querySpecialPartPage(params)
            let {total} = await ctx.service.specialPart.queryAllCountSpecialPart(params)
            let newResult = {
                result,
                total,
                currentPage,
    ...
    async updateSpecialPartByUid() {
        ...
        if (!isValid) { // 校验不通过
            ...
            // 补充参数
            let {create_time} = await ctx.service.specialPart.querySpecialPartByUid(uid)
            params.createTime = create_time

            // 根据uid修改这条记录
            let result = await ctx.service.specialPart.updateSpecialPartByUid(params)
            if (result) {
                ctx.success()
    ...
    async querySpecialPartAll() {
        const {ctx} = this
        console.log(ctx.request.body, 'ctx.request.body')
        let result = await ctx.service.specialPart.querySpecialPartAll()
        ctx.success(result)
    }

}

module.exports = SpecialPartController;
