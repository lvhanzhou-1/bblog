...
class SpecialController extends Controller {
    ...
    async saveSpecial() {
        ...
        if (!isValid) { // 校验不通过
             ...
             *
             */
            let result = await ctx.service.special.querySpecialBySpecialName(specialName)
            ...
            if (result) { // 已存在，不保存
                ...
                let flag = await ctx.service.special.saveSpecial(params)
                if (flag) {
                    ctx.success()
    ...
    async deleteSpecialByUid() {
        ...
        if (!isValid) { // 校验不通过
            ...
            let result = await ctx.service.special.deleteSpecialByUid(uids)
            if (result) {
                ctx.success()
            }
        }
    ...
    async querySpecialPage() {
        ...
        if (!isValid) { // 校验不通过
            ...
            let result = await ctx.service.special.querySpecialPage(params)
            let {total} = await ctx.service.special.queryAllCountSpecial(params)
            let newResult = {
                result,
                total,
                currentPage,
    ...
    async updateSpecialByUid() {
        ...
        if (!isValid) { // 校验不通过
            ...
            // 补充参数
            let {create_time} = await ctx.service.special.querySpecialByUid(uid)
            params.createTime = create_time

            // 根据uid修改这条记录
            let result = await ctx.service.special.updateSpecialByUid(params)
            if (result) {
                ctx.success()
    ...
    async querySpecialAll() {
        const {ctx} = this
        // 判断是否是管理员，为了排除私密文章
        let isXzzOrCheny = await this.isXzzOrCheny(ctx)
        console.log(ctx.request.body, 'ctx.request.body')
        let result = await ctx.service.special.querySpecialAll(isXzzOrCheny)
        ctx.success(result)
    }
