...
class AdminRoleController extends Controller {
    ...
    async saveAdminRole() {
        ...
        if (!isValid) { // 校验不通过
             ...
             *
             */
            let result = await ctx.service.adminRole.queryAdminRoleByRoleName(roleName)
            ...
            if (result) { // 已存在，不保存
                ...
                let flag = await ctx.service.adminRole.saveAdminRole(params)
                if (flag) {
                    ctx.success()
                }
            }
    ...
    async deleteAdminRoleByUid() {
        ...
        if (!isValid) { // 校验不通过
            ...
            let result = await ctx.service.adminRole.deleteAdminRoleByUid(uids)
            if (result) {
                ctx.success()
            }
        }
    ...
    async queryAdminRolePage() {
        ...
        if (!isValid) { // 校验不通过
            ...
            let result = await ctx.service.adminRole.queryAdminRolePage(params)
            let {total} = await ctx.service.adminRole.queryAllCountAdminRole(params)
            let newResult = {
                result,
                total,
                currentPage,
    ...
    async updateAdminRoleByUid() {
        ...
        if (!isValid) { // 校验不通过
            ...
            // 补充参数
            let {create_time} = await ctx.service.adminRole.queryAdminRoleByUid(uid)
            params.createTime = create_time

            // 根据uid修改这条记录
            let result = await ctx.service.adminRole.updateAdminRoleByUid(params)
            if (result) {
                ctx.success()
            }
        }
    ...
    async queryAdminRoleAll() {
        const {ctx} = this
        console.log(ctx.request.body, 'ctx.request.body')
        let result = await ctx.service.adminRole.queryAdminRoleAll()
        ctx.success( result)
    }
}
