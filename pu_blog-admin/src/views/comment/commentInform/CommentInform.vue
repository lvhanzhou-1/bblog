				...
				<el-row>
					<el-col :span="12">
						<el-form-item
							label="被举报评论"
							label-width="120px"
							prop="informCommentId"
						>
							<el-input
								v-model="form.informCommentId"
								auto-complete="off"
							></el-input>
						</el-form-item>
					</el-col>
    ...
    */
	...
	methods: {
		...
		async deleteConfirmHandle() {
			let { data } = await commentInformApi.deleteCommentInformByUid(
				this.selectIds
			);
			if (data.code === 1) {
				this.$message({
		...
		async getList() {
			let { data } = await commentInformApi.queryCommentInformPage(
				this.searchParam
			);
