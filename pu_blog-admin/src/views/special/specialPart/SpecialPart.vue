...
*/
import { specialPartApi } from "@/api/specialPart";
    ...
    */
	...
	methods: {
		...
		async deleteConfirmHandle() {
			let { data } = await specialPartApi.deleteSpecialPartByUid(
				this.selectIds
			);
			if (data.code === 1) {
				this.$message({
		...
		async getList() {
			let { data } = await specialPartApi.querySpecialPartPage(
				this.searchParam
			);
