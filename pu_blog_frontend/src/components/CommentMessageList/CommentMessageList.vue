<template>
    <div style="margin-top: 10px">
        ...
        <div v-if="commentsList.length === 0" class="empty-box">
            <IconFont :icon-class="{width:'30px',height:'30px'}" :icon-href="'icon-kongji'"></IconFont>
        </div>

        <!--        遍历评论数组 每一条评论就是一个组件-->
        <div v-else class="message-container" v-for="(item,index) in commentsList" :key="item.uid">
            <!--   隐藏按钮  如果层级大于1就显示，第一层不显示 -->
            <div v-if="level>1" class="vertical-line" v-show="item.currentCommentIsShow">
                <!--  点击切换隐藏或显示，通过改变currentCommentIsShow-->
                <a href="javascript:void(0)" onclick="return false" class="minus-box"
                   @click="hiddenOrShowComment(item.uid,'hidden')">
                    <IconFont :icon-class="{width:'16px',height:'16px'}" :icon-href="'icon-jianshao'"></IconFont>
                </a>
            </div>
            ...
            <div v-if="level>1" class="vertical-line" v-show="!item.currentCommentIsShow">
                <a href="javascript:void(0)" onclick="return false" class="minus-box"
                   @click="hiddenOrShowComment(item.uid,'show')">
                    <IconFont :icon-class="{width:'16px',height:'16px'}" :icon-href="'icon-jiahao'"></IconFont>
                </a>
            </div>

            <!--   隐藏后的提示语   -->
            <div v-show="!item.currentCommentIsShow" class="text-tips" @click="hiddenOrShowComment(item.uid,'show')">
                展开被隐藏评论
            </div>
...
<!--                              :icon-href="`icon-${item.comment_person_profile}`"></IconFont>-->
<!--                </div>-->
            ...
            <div v-show="item.currentCommentIsShow" class="right-content">
                ...
                <div class="comment-box">
                    <div class="comment-head">
                        <span class="comment-name">{{ item.comment_person_name }}</span>
                        <span class="label" v-if="item.comment_person_id === adminUserId">
                            博主
                        </span>

                        <span>{{ timeListStringRange[index] }}</span>


                    </div>
                    <!-- 评论内容 确定是否可公开-->
                    <div class="comment-content" :class="[{'illegal': +(item.comment_status)===3},'message']">
                        {{ new_comment_content(item.comment_content, item.comment_person_id) }}
                    </div>

                    <!-- 评论level -->
                    <i class="fa fa-heart">level：{{ level }}</i>
                    <i class="fa fa-heart" @click="reportComment(item)">举报</i>
                    <i class="fa fa-heart" @click="deleteComment(item)" v-if="isShowDeleteBtn(item)">删除</i>
                    <i class="fa fa-reply" @click="showReplayBox(item)" v-if="level<maxReplyLevel">回复</i>
...
import {adminUserId} from "@/config/config";
...
export default {
    ...
    methods: {
        ...
        /*转码, 后面需要改一下*/
        new_comment_content(s, id) {
            // if (id === '82405310-1f03-11ed-9d04-97ea178f9a35') {
            return decodeURI(window.atob(s))
            // }
            // return s
        },
    },
