<template>
    <div class="follow-us-container">
        <h2>{{ title }}</h2>
        <ul>
            <li v-for="item in list" :key="item.uid" :style="{border:`1px solid ${item.icon_color}`}"
                @click="contactMeHandle(item)">
                <span class="icon-box" :style="{color: item.icon_color}">
                </span>
                <span class="content">{{ item.way_num }}</span>
                <span class="label" :style="{background: item.icon_color}">
                    {{ item.contact_way }}
                </span>
            </li>
        </ul>
